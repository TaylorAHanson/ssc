...
CURRENT_SCOPES=$(databricks apps get "$APP_NAME" 2>/dev/null | python -c "import sys, json; d=json.load(sys.stdin); print(json.dumps(sorted(d.get('effective_user_api_scopes', []))) if d and 'effective_user_api_scopes' in d else 'null')" 2>/dev/null || echo "null")
TARGET_SCOPES=$(echo "$SCOPES_LIST" | python -c "import sys, json; print(json.dumps(sorted(json.load(sys.stdin))))")
