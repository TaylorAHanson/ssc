...
export function ServicePrincipalForm() {
  ...
  const { survey, isLoading, error } = useFormLoader('/paas/service-principal');
  ...
  useEffect(() => {
    ...
    if (prefillData) {
      try {
        ...
        if (data.application_name) formData.application_name = data.application_name;
        if (data.permissions) {
          formData.permissions = Array.isArray(data.permissions) ? data.permissions : [data.permissions];
        }
