...
interface DiscoverSearchProps {
  ...
  /** `hero`: a large, always-open search bar (used on the Discover landing page). */
  variant?: 'compact' | 'hero';
  /** Compact only: stay open instead of collapsing to a "Search" button. */
  alwaysOpen?: boolean;
}
...
export function DiscoverSearch({
  ...
  const [expandedState, setIsExpanded] = useState<boolean>(() => Boolean(value));
  const isExpanded = isHero || alwaysOpen || expandedState;
    ...
    return (
      <form onSubmit={handleSubmit} className={`relative flex items-center ${className}`}>
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Escape' && localValue) handleClear();
          }}
          placeholder={placeholder}
          aria-label="Search catalog or ask AI"
          className="w-full pl-12 pr-36 py-3.5 bg-white border border-slate-300 rounded-2xl text-sm text-slate-800 placeholder-slate-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
        />
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
          ...
          {onSubmitAgentQuery && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-primary hover:bg-primary/90 disabled:opacity-40 text-white text-xs font-semibold transition-all cursor-pointer"
              title="Ask AI (Enter)"
            >
