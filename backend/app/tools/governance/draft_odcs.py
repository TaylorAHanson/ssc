...
class DraftOdcsInput(BaseModel):
    dataset_id: str = Field(..., description="The fully qualified dataset ID (e.g., catalog.schema.table)")
...
@tool(
    ...
    description="Drafts an Open Data Contract Standard (ODCS) v3 YAML document for a given dataset by fetching its metadata and using AI to generate the structure. Important: Always use this tool to generate the 'odcs_yaml' parameter before calling execute_workflow for data_certification.",
    args_schema=DraftOdcsInput
)
async def draft_odcs_contract(dataset_id: str, violations_context: Optional[Any] = None) -> str:
    """
    Fetches table metadata and generates a draft ODCS YAML using the AgentLLMClient.
    """
    import ast
    ...
    try:
        ...
        if len(parts) != 3:
            raise ValueError(f"Invalid dataset_id format: {dataset_id}. Expected catalog.schema.table")
        ...
        # 3. Assess the current policy state
        # We fetch the violation context from OPA if it exists to help the LLM fix the issues
        # Actually, if we are passing this from a prompt, we can inject OPA violations if known.
        # For now, we will add an explicit instruction.
        ...
        prompt = f"""
You are an expert Data Architect. Generate a valid Open Data Contract Standard (ODCS) v3 YAML document based on the following table metadata.
...
dataProduct: <dataset physical name>
version: 1.0.0
status: active
...
description:
  purpose: <combine catalog and schema descriptions or infer from name>
...
quality:
  - id: technical_dq_threshold
    ...
    mustBe: <infer reasonable threshold (e.g. 90, 95, 100) based on actual TDQ Score if available, otherwise 100>
  - id: business_dq_threshold
    type: custom
    engine: acceldata
    description: "Business logic validation score threshold"
    mustBe: <infer reasonable threshold based on actual BDQ Score if available, otherwise 100>
  - id: schema_drift
    type: custom
    engine: acceldata
...
schema:
  - id: <dataset physical name>_obj
    ...
    description: <schema description or infer from name>
    ...
    customProperties:
      ...
      - property: classification
        value: PII
    properties:
      # FOREACH column in the Columns metadata above, generate a property block like this:
      - id: <column name>
        name: <column name>
