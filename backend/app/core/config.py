...
class Settings(BaseSettings):
    ...
    # DISABLED BY DEFAULT (0). This wall-clock cap abandons a workspace whose scan
    # exceeds it, contributing ZERO findings for that workspace — which on a large
    # workspace (many notebooks/jobs) that legitimately takes many minutes looks
    # like the sentinel suddenly "detecting <1% of failures". Before this cap
    # existed the scan simply ran to completion (slow but correct). True infinite
    # hangs are ALREADY prevented by SENTINEL_SDK_HTTP_TIMEOUT_SECONDS (every SDK
    # call is individually bounded), so the wall-clock cap is not needed to keep
    # the app alive and only causes false-negative "timeout" failures. Leave it 0
    # unless you have a specific runaway workspace to bound; set a generous value
    # (e.g. 3600) as a pure backstop if so.
    SENTINEL_WORKSPACE_SCAN_TIMEOUT_SECONDS: int = int(
        os.getenv("SENTINEL_WORKSPACE_SCAN_TIMEOUT_SECONDS", "0")
    )
