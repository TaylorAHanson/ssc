...
**Pick the reviewer's model deliberately** (*Reviewer model* / *Reviewer reasoning effort*; blank = the agent's model). A fast chat model with reasoning off (e.g. `gpt-6-luna`, which rejects tool calls with reasoning on) skims a mid-size repo in about 30 seconds and is decisive but misses some depth; a model that reasons while calling tools (e.g. a Claude Sonnet/Opus reference) traces the code, takes a few minutes, and misses different things. Neither is exhaustive: the report is triage for the security admin, not a substitute for their review.
