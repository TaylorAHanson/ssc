...
**Pick the reviewer's model deliberately** (*Reviewer model* / *Reviewer reasoning effort*; blank = the agent's model). A fast chat model with reasoning off (e.g. `gpt-6-luna`, which rejects tool calls with reasoning on) skims a mid-size repo in about 30 seconds and is decisive but misses some depth; a fast model that reasons while calling tools (e.g. Gemini Flash with reasoning `medium`) takes a minute or two and traces more; a Claude Sonnet/Opus reference takes several minutes and misses different things. Neither is exhaustive: the report is triage for the security admin, not a substitute for their review.

Tune it under **Admin → Settings → App Code Review**: the reviewer rubric (the security team's controls and exception triggers; its *Terminology* block is where you name your platform, the external services apps may already use, your exception process and data classes), reading rounds, a time limit, and size limits. The GitHub token needs read access to the submitted repos ([GitHub Token Permissions](./GITHUB_TOKEN_PERMISSIONS.md)).
