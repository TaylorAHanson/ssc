...
| 2026‑07‑22 12:xx | **Finding 4 fix (large‑run SSL drop on completion):** enforcement no longer writes the full per‑violation `enforcement_actions` list back into `requests.state_context` (nor returns it through the `sentinel_enforce` tool). On a big prod run (~58k findings) that inline multi‑MB single‑row JSON `UPDATE` dropped the Lakebase connection ("SSL connection has been closed unexpectedly"), poisoning the session so the terminal `UPDATE requests SET status='completed'` failed and the finished run was reported **FAILED** — the same giant‑write failure the `sentinel_findings` table was built to avoid. Now `state_context` gets only compact `enforcement_stats` (per‑action detail already lives one‑row‑each in `enforcement_audit`, which the `/enforcement-actions` API reads), and audit rows are **committed in batches of 1000** during the loop so a huge run is neither one giant transaction nor an idle‑for‑minutes connection. `backend/app/workflows/sentinel.py` (`run_enforcement`). | _pending commit_ | ⏳ Pending prod re‑run; unit suite green (`test_sentinel_multiworkspace`, `test_evaluator`) |
