...
A key component of our governance framework is the proactive certification of data assets. Governance is uniform across Self-Service Hub, meaning all eligible production tables must be pushed through certification. To minimize manual entry, we use an AI-assisted, proactive workflow.
...
*   **Multi-Workspace Scope:** A single Sentinel run scans **every configured target workspace** (Admin → Settings → Target Workspaces) for workspace-scoped resources (compute, jobs, apps, warehouses, dashboards, etc.), aggregating all findings into one report and one governance email. Each violation is tagged with the workspace it came from (shown as a **Workspace** column in the report and email). A manual run can be scoped to a single workspace via the Sentinel page's Advanced options; the default is **All target workspaces**. Because Unity Catalog is metastore- (not workspace-) scoped, the **data certification** pass runs once against a single workspace — the app's home workspace by default, or the workspace named in the `SENTINEL_DATA_CERT_WORKSPACE` setting (Admin → Settings → Target Workspaces). The DQ warehouse + ADOC schema always come from the global settings.
